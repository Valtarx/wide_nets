...
def visualize_search(nodeID, GUID):
    ...
    plt.savefig('images/img0.png', dpi=300, bbox_inches='tight')
    ...
    for level in range(1, 40):
        ...
        nx.draw(G, pos, **options)
        plt.savefig('images/img{0}.png'.format(level), dpi=300, bbox_inches='tight')
        if color_map[node] == 'blue':
            frames = []
            ...
            frames[0].save('images/png_to_gif.gif', format='GIF',
...
visualize_search(random.randint(0, number_of_nodes - 1), files[server])
...
if number_of_nodes == errors:
    ...
    print('Количество дуг, задействованных в поиске:\n'
          '{0}/{1}/{2}'.format(min, avg, max))
...
print('Объем хранимой информации на узле (в количестве записей):\n'
      '{0}/{1}/{2}'.format(min, avg, max))

print('Количество неудачных запросов: {}'.format(errors))
